<?php
/**
 * File containing a patched workflow.php cronjob - original taken from 5.1
 *
 */
...
// Store changes to process
    ...
    {   //restore memento and run it
        ...
        // patch wrt original runcronjobs
        $original_user = eZUser::fetch( $original_user_id = $process->attribute( 'user_id' ) );
        if ( $original_user )
        {
            /// @todo what if original user has been disabled in the meantime?
            $logged_in_user = eZUser::setCurrentlyLoggedInUser( $original_user, $original_user_id );
            $cli->output( "Logged in as original user: ". $original_user->attribute( 'login' ) );
        }
        else
        {
            eZDebug::writeError( "User $original_user_id has been deleted and can not be used for finishing workflow", __METHOD__ );
        }
        // /patch
